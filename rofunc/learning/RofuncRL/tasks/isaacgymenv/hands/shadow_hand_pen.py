...
class ShadowHandPenTask(VecTask):
    """
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[
            ...
            self.pen_right_handle_pos, self.pen_left_handle_pos,
...
def compute_hand_reward(
    ...
    left_hand_finger_dist = (torch.norm(pen_left_handle_pos - left_hand_ff_pos, p=2, dim=-1) + torch.norm(
        pen_left_handle_pos - left_hand_mf_pos, p=2, dim=-1)
                             + torch.norm(pen_left_handle_pos - left_hand_rf_pos, p=2, dim=-1) + torch.norm(
                pen_left_handle_pos - left_hand_lf_pos, p=2, dim=-1)
    ...
    successes = torch.where(successes == 0,
                            torch.where(torch.norm(pen_right_handle_pos - pen_left_handle_pos, p=2, dim=-1) * 5 > 1.5,
