...
class ShadowHandMetaMT4(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for task_env in self.task_envs:
            hand_start_pose, another_hand_start_pose, object_pose, goal_pose, another_object_pose, another_goal_pose, table_pose_dim, object_asset_options, object_type = obtain_task_info(
                task_env)
            ...
            another_object_start_pose.p = gymapi.Vec3(another_object_pose[0], another_object_pose[1],
                                                      another_object_pose[2])
            another_object_start_pose.r = gymapi.Quat().from_euler_zyx(another_object_pose[3], another_object_pose[4],
                                                                       another_object_pose[5])
            ...
            another_goal_start_pose.r = gymapi.Quat().from_euler_zyx(another_goal_pose[3], another_goal_pose[4],
                                                                     another_goal_pose[5])
            ...
            another_object_assets.append(self.gym.load_asset(self.sim, asset_root, self.asset_files_dict[object_type],
        ...
        for env_id in range(self.num_tasks):
            for i in range(self.num_each_envs):
                ...
                goal_handle = self.gym.create_actor(env_ptr, goal_assets[env_id], goal_start_poses[env_id],
                                                    "goal_object", index + self.num_envs, 0, 0)
                ...
                # add another object
                object_another_handle = self.gym.create_actor(env_ptr, another_object_assets[env_id],
                                                              another_object_start_poses[env_id], "another_object",
                                                              index, 0, 0)
                self.another_object_init_state.append(
                    [another_object_start_poses[env_id].p.x, another_object_start_poses[env_id].p.y,
                     another_object_start_poses[env_id].p.z,
                     another_object_start_poses[env_id].r.x, another_object_start_poses[env_id].r.y,
                     another_object_start_poses[env_id].r.z, another_object_start_poses[env_id].r.w,
                     0, 0, 0, 0, 0, 0])
                ...
                goal_another_handle = self.gym.create_actor(env_ptr, another_goal_assets[env_id],
                                                            another_goal_start_poses[env_id], "another_goal_object",
                                                            index + self.num_envs * 2, 0, 0)
                self.another_goal_init_state.append(
                    [another_goal_start_poses[env_id].p.x, another_goal_start_poses[env_id].p.y,
                     another_goal_start_poses[env_id].p.z,
                     another_goal_start_poses[env_id].r.x, another_goal_start_poses[env_id].r.y,
                     another_goal_start_poses[env_id].r.z, another_goal_start_poses[env_id].r.w,
                     0, 0, 0, 0, 0, 0])
        ...
        self.goal_init_state = to_torch(self.goal_init_state, device=self.device, dtype=torch.float).view(self.num_envs,
                                                                                                          13)
        self.another_object_init_state = to_torch(self.another_object_init_state, device=self.device,
                                                  dtype=torch.float).view(self.num_envs, 13)
        self.another_goal_init_state = to_torch(self.another_goal_init_state, device=self.device,
                                                dtype=torch.float).view(self.num_envs, 13)
        ...
        self.another_goal_object_indices = to_torch(self.another_goal_object_indices, dtype=torch.long,
                                                    device=self.device)
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[
            ...
            self.another_object_pos, self.another_object_rot, self.another_goal_pos, self.another_goal_rot,
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[:,
        another_obj_obs_start + 10:another_obj_obs_start + 13] = self.vel_obs_scale * self.another_object_angvel
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):
        ...
        self.root_state_tensor[self.another_goal_object_indices[env_ids], 7:13] = torch.zeros_like(
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_relative_control:
            ...
            self.gym.apply_rigid_body_force_tensors(self.sim, gymtorch.unwrap_tensor(self.total_apply_force),
                                                    gymtorch.unwrap_tensor(self.total_apply_torque), gymapi.ENV_SPACE)
