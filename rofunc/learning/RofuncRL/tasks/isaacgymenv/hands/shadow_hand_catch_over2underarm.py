...
class ShadowHandCatchOver2UnderarmTask(VecTask):
    """
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            if self.obs_type in ["point_cloud"]:
                ...
                self.gym.set_camera_location(camera_handle, env_ptr, gymapi.Vec3(0.4, -0.25, 1),
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[
            ...
            self.hand_positions[self.another_hand_indices, :], self.hand_positions[self.hand_indices, :],
            self.dist_reward_scale, self.rot_reward_scale, self.rot_eps, self.actions, self.action_penalty_scale,
            self.success_tolerance, self.reach_goal_bonus, self.fall_dist, self.fall_penalty,
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_relative_control:
            targets = self.prev_targets[:,
                      self.actuated_dof_indices] + self.shadow_hand_dof_speed_scale * self.dt * self.actions[:, 6:26]
            self.cur_targets[:, self.actuated_dof_indices] = tensor_clamp(targets,
                                                                          ...
                                                                          self.shadow_hand_dof_upper_limits[
                                                                              self.actuated_dof_indices])
            targets = self.prev_targets[:,
                      self.actuated_dof_indices + 24] + self.shadow_hand_dof_speed_scale * self.dt * self.actions[:,
                                                                                                     32:52]
...
def compute_hand_reward(
    ...
    right_hand_base_dist = torch.norm(
        right_hand_base_pos - torch.tensor([0.0, 0.0, 0.5], dtype=torch.float, device=device), p=2, dim=-1)
    left_hand_base_dist = torch.norm(
        left_hand_base_pos - torch.tensor([0.0, -0.8, 0.5], dtype=torch.float, device=device), p=2, dim=-1)
