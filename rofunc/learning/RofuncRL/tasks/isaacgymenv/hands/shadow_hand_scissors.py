...
class ShadowHandScissorsTask(VecTask):
    """
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[
            ...
            self.scissors_right_handle_pos, self.scissors_left_handle_pos, self.object_dof_pos,
    ...
    def compute_observations(self):
        ...
        self.scissors_right_handle_pos = self.scissors_right_handle_pos + quat_apply(self.scissors_right_handle_rot,
                                                                                     to_torch([0, 1, 0],
        ...
        self.scissors_left_handle_pos = self.scissors_left_handle_pos + quat_apply(self.scissors_left_handle_rot,
                                                                                   to_torch([0, 1, 0],
                                                                                            device=self.device).repeat(
                                                                                       self.num_envs, 1) * 0.0)
        self.scissors_left_handle_pos = self.scissors_left_handle_pos + quat_apply(self.scissors_left_handle_rot,
    ...
    def reset_idx(self, env_ids, goal_env_ids):
        ...
        self.prev_targets[env_ids, self.num_shadow_hand_dofs * 2:self.num_shadow_hand_dofs * 2 + 2] = to_torch([-0.59],
                                                                                                               device=self.device)
        self.cur_targets[env_ids, self.num_shadow_hand_dofs * 2:self.num_shadow_hand_dofs * 2 + 2] = to_torch([-0.59],
                                                                                                              device=self.device)
...
def compute_hand_reward(
    ...
    right_hand_finger_dist = (torch.norm(scissors_right_handle_pos - right_hand_ff_pos, p=2, dim=-1) + torch.norm(
        scissors_right_handle_pos - right_hand_mf_pos, p=2, dim=-1)
                              + torch.norm(scissors_right_handle_pos - right_hand_rf_pos, p=2, dim=-1) + torch.norm(
                scissors_right_handle_pos - right_hand_lf_pos, p=2, dim=-1)
                              + torch.norm(scissors_right_handle_pos - right_hand_th_pos, p=2, dim=-1))
    left_hand_finger_dist = (torch.norm(scissors_left_handle_pos - left_hand_ff_pos, p=2, dim=-1) + torch.norm(
        scissors_left_handle_pos - left_hand_mf_pos, p=2, dim=-1)
                             + torch.norm(scissors_left_handle_pos - left_hand_rf_pos, p=2, dim=-1) + torch.norm(
                scissors_left_handle_pos - left_hand_lf_pos, p=2, dim=-1)
