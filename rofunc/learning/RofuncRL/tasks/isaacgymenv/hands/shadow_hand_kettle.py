...
class ShadowHandKettle(VecTask):
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[
            ...
            self.kettle_handle_pos, self.bucket_handle_pos, self.kettle_spout_pos,
    ...
    def compute_observations(self):
        ...
        self.kettle_handle_pos = self.kettle_handle_pos + quat_apply(self.kettle_handle_rot,
        ...
        self.kettle_spout_pos = self.kettle_spout_pos + quat_apply(self.kettle_spout_rot,
                                                                   to_torch([0, 1, 0], device=self.device).repeat(
                                                                       self.num_envs, 1) * 0.0)
        self.kettle_spout_pos = self.kettle_spout_pos + quat_apply(self.kettle_spout_rot,
                                                                   to_torch([1, 0, 0], device=self.device).repeat(
        ...
        self.bucket_handle_pos = self.bucket_handle_pos + quat_apply(self.bucket_handle_rot,
                                                                     to_torch([0, 1, 0], device=self.device).repeat(
                                                                         self.num_envs, 1) * 0.0)
        self.bucket_handle_pos = self.bucket_handle_pos + quat_apply(self.bucket_handle_rot,
                                                                     to_torch([1, 0, 0], device=self.device).repeat(
...
def compute_hand_reward(
        ...
        max_episode_length: float, object_pos, object_rot, target_pos, target_rot, kettle_handle_pos, bucket_handle_pos,
        kettle_spout_pos,
    ...
    right_hand_finger_dist = (torch.norm(kettle_handle_pos - right_hand_ff_pos, p=2, dim=-1) + torch.norm(
        kettle_handle_pos - right_hand_mf_pos, p=2, dim=-1)
                              + torch.norm(kettle_handle_pos - right_hand_rf_pos, p=2, dim=-1) + torch.norm(
                kettle_handle_pos - right_hand_lf_pos, p=2, dim=-1)
                              + torch.norm(kettle_handle_pos - right_hand_th_pos, p=2, dim=-1))
    left_hand_finger_dist = (torch.norm(bucket_handle_pos - left_hand_ff_pos, p=2, dim=-1) + torch.norm(
        bucket_handle_pos - left_hand_mf_pos, p=2, dim=-1)
                             + torch.norm(bucket_handle_pos - left_hand_rf_pos, p=2, dim=-1) + torch.norm(
                bucket_handle_pos - left_hand_lf_pos, p=2, dim=-1)
    ...
    up_rew = torch.where(right_hand_finger_dist < 0.7,
                         torch.where(left_hand_finger_dist < 0.7,
                                     0.5 - torch.norm(bucket_handle_pos - kettle_spout_pos, p=2, dim=-1) * 2, up_rew),
                         up_rew)
