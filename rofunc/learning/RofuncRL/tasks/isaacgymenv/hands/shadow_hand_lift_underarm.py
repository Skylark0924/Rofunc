...
class ShadowHandLiftUnderarmTask(VecTask):
    """
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            if self.obs_type in ["point_cloud"]:
                ...
                self.gym.set_camera_location(camera_handle, env_ptr, gymapi.Vec3(0.25, -0.57, 0.75),
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[
            ...
            self.pot_left_handle_pos, self.pot_right_handle_pos,
            self.left_hand_pos, self.right_hand_pos,
            self.dist_reward_scale, self.rot_reward_scale, self.rot_eps, self.actions, self.action_penalty_scale,
            self.success_tolerance, self.reach_goal_bonus, self.fall_dist, self.fall_penalty,
            self.max_consecutive_successes, self.av_factor, (self.object_type == "pen")
    ...
    def compute_observations(self):
        ...
        self.pot_right_handle_pos = self.object_pos + quat_apply(self.object_rot,
                                                                 to_torch([0, 1, 0], device=self.device).repeat(
                                                                     self.num_envs, 1) * 0.15)
        self.pot_right_handle_pos = self.pot_right_handle_pos + quat_apply(self.object_rot, to_torch([0, 0, 1],
                                                                                                     device=self.device).repeat(
            self.num_envs, 1) * 0.06)
        self.pot_left_handle_pos = self.object_pos + quat_apply(self.object_rot,
                                                                to_torch([0, 1, 0], device=self.device).repeat(
                                                                    self.num_envs, 1) * -0.15)
        self.pot_left_handle_pos = self.pot_left_handle_pos + quat_apply(self.object_rot,
    ...
    def reset_idx(self, env_ids, goal_env_ids):
        ...
        self.hand_linvels[all_hand_indices.to(torch.long), :] = self.saved_root_tensor[all_hand_indices.to(torch.long),
                                                                7:10]
        self.hand_angvels[all_hand_indices.to(torch.long), :] = self.saved_root_tensor[all_hand_indices.to(torch.long),
                                                                10:13]
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                ...
                pot_left_handle_posx = (self.pot_left_handle_pos[i] + quat_apply(self.object_rot[i], to_torch([1, 0, 0],
                                                                                                              device=self.device) * 0.2)).cpu().numpy()
                pot_left_handle_posy = (self.pot_left_handle_pos[i] + quat_apply(self.object_rot[i], to_torch([0, 1, 0],
                                                                                                              device=self.device) * 0.2)).cpu().numpy()
                pot_left_handle_posz = (self.pot_left_handle_pos[i] + quat_apply(self.object_rot[i], to_torch([0, 0, 1],
                                                                                                              device=self.device) * 0.2)).cpu().numpy()

                p0 = self.pot_left_handle_pos[i].cpu().numpy()
                self.gym.add_lines(self.viewer, self.envs[i], 1,
                                   [p0[0], p0[1], p0[2], pot_left_handle_posx[0], pot_left_handle_posx[1],
                                    pot_left_handle_posx[2]], [0.85, 0.1, 0.1])
                self.gym.add_lines(self.viewer, self.envs[i], 1,
                                   [p0[0], p0[1], p0[2], pot_left_handle_posy[0], pot_left_handle_posy[1],
                                    pot_left_handle_posy[2]], [0.1, 0.85, 0.1])
                self.gym.add_lines(self.viewer, self.envs[i], 1,
                                   [p0[0], p0[1], p0[2], pot_left_handle_posz[0], pot_left_handle_posz[1],
                                    pot_left_handle_posz[2]], [0.1, 0.1, 0.85])

                pot_right_handle_posx = (self.pot_right_handle_pos[i] + quat_apply(self.object_rot[i],
                                                                                   to_torch([1, 0, 0],
                                                                                            device=self.device) * 0.2)).cpu().numpy()
                pot_right_handle_posy = (self.pot_right_handle_pos[i] + quat_apply(self.object_rot[i],
                                                                                   to_torch([0, 1, 0],
                                                                                            device=self.device) * 0.2)).cpu().numpy()
                pot_right_handle_posz = (self.pot_right_handle_pos[i] + quat_apply(self.object_rot[i],
                                                                                   to_torch([0, 0, 1],
                                                                                            device=self.device) * 0.2)).cpu().numpy()

                p0 = self.pot_right_handle_pos[i].cpu().numpy()
                self.gym.add_lines(self.viewer, self.envs[i], 1,
                                   [p0[0], p0[1], p0[2], pot_right_handle_posx[0], pot_right_handle_posx[1],
                                    pot_right_handle_posx[2]], [0.85, 0.1, 0.1])
                self.gym.add_lines(self.viewer, self.envs[i], 1,
                                   [p0[0], p0[1], p0[2], pot_right_handle_posy[0], pot_right_handle_posy[1],
                                    pot_right_handle_posy[2]], [0.1, 0.85, 0.1])
                self.gym.add_lines(self.viewer, self.envs[i], 1,
                                   [p0[0], p0[1], p0[2], pot_right_handle_posz[0], pot_right_handle_posz[1],
                                    pot_right_handle_posz[2]], [0.1, 0.1, 0.85])
