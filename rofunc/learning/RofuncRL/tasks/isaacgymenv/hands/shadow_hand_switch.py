...
class ShadowHandSwitchTask(VecTask):
    """
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture,
        ...
        self.dof_force_tensor = gymtorch.wrap_tensor(dof_force_tensor).view(self.num_envs,
                                                                            self.num_shadow_hand_dofs * 2 + self.num_object_dofs * 2 + 1)
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[
            ...
            self.switch_right_handle_pos, self.switch_left_handle_pos,
    ...
    def compute_observations(self):
        ...
        self.switch_right_handle_pos = self.switch_right_handle_pos + quat_apply(self.switch_right_handle_rot,
                                                                                 to_torch([0, 1, 0],
        ...
        self.switch_left_handle_pos = self.switch_left_handle_pos + quat_apply(self.switch_left_handle_rot,
                                                                               to_torch([0, 1, 0],
                                                                                        device=self.device).repeat(
                                                                                   self.num_envs, 1) * -0.02)
        self.switch_left_handle_pos = self.switch_left_handle_pos + quat_apply(self.switch_left_handle_rot,
...
def compute_hand_reward(
    ...
    right_hand_finger_dist = (torch.norm(switch_right_handle_pos - right_hand_ff_pos, p=2, dim=-1) + torch.norm(
        switch_right_handle_pos - right_hand_mf_pos, p=2, dim=-1)
                              + torch.norm(switch_right_handle_pos - right_hand_rf_pos, p=2, dim=-1) + torch.norm(
                switch_right_handle_pos - right_hand_lf_pos, p=2, dim=-1)
                              + torch.norm(switch_right_handle_pos - right_hand_th_pos, p=2, dim=-1))
    left_hand_finger_dist = (torch.norm(switch_left_handle_pos - left_hand_ff_pos, p=2, dim=-1) + torch.norm(
        switch_left_handle_pos - left_hand_mf_pos, p=2, dim=-1)
                             + torch.norm(switch_left_handle_pos - left_hand_rf_pos, p=2, dim=-1) + torch.norm(
                switch_left_handle_pos - left_hand_lf_pos, p=2, dim=-1)
    ...
    successes = torch.where(successes == 0,
                            torch.where(1.4 - (switch_right_handle_pos[:, 2] + switch_left_handle_pos[:, 2]) > 0.05,
