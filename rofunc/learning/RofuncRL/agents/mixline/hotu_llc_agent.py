...
class HOTULLCAgent(ASEAgent):
    ...
    def _build_decompose_model(self):
        ...
        for i in range(1, self.num_parts):
            ...
            self.checkpoint_modules[f"discriminator_{i}"].append(disc_model)
            self.checkpoint_modules[f"encoder_{i}"].append(enc_model)
            ...
            self.checkpoint_modules[f"optimizer_disc_{i}"].append(optimizer_disc)
            self.checkpoint_modules[f"optimizer_enc_{i}"].append(optimizer_enc)
            self.checkpoint_modules[f"amp_state_preprocessor_{i}"].append(amp_state_preprocessor)
