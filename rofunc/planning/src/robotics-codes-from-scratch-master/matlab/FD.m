%%	  Forward dynamics in matrix form
%%
...
param.l = [1.4, .8, .9]; %Robot links lengths
param.m = [1.2, 1.5, .3]; %Robot links masses
...
x = [0; 0; 0; zeros(param.nbVarX,1)]; %Initial robot state (position and velocity) 
...
for t=1:nbData-1	
	...
	%G = -sum(Tm,2) .* l' .* cos(T*x(1:nbDOFs,t)) * param.g;
	%G = T' * G;
	G = -T' * diag(l) * Tm * cos(T*x(1:nbDOFs,t)) * param.g;
