...
def _run_sim(motion):
    body_links = {
            "hand_l": gymapi.AXIS_TRANSLATION, "hand_r": gymapi.AXIS_TRANSLATION,
                  ...
                  "hip_pitch_link_l": gymapi.AXIS_ROTATION, "hip_pitch_link_r": gymapi.AXIS_ROTATION,
...
def motion_retargeting(retarget_cfg, source_motion, visualize=False):
    ...
    if visualize:
        rf.logger.beauty_print("Plot Bruce T-pose", type="module")
    ...
    rf.logger.beauty_print(f"Save Bruce dof_states to {retarget_cfg['target_motion_path']}", type="module")
...
def npy_from_fbx(fbx_file):
    ...
    config = {
        ...
        "target_dof_states_path": fbx_file.replace('_optitrack.fbx', '_optitrack2bruce_dof_states.npy'),
        ...
        "scale": 0.0005,  # Export millimeter to meter
