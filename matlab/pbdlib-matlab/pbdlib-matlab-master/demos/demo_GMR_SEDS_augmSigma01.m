function demo_GMR_SEDS_augmSigma01
% Continuous autonomous dynamical system with augmented state-space encoding and  
...
rData0(:,1) = Data0;
for t=2:nbData
	%dx = GMR(model0, rData0(:,t-1), [1:2], [3:4]); 
	dx = GMR(model, [rData0(:,t-1); 1], in, out); 
...
for t=2:nbData
  ...
  %Compute velocity command through GMR
	dx = 0;
  for i=1:model.nbStates
    dx = dx + h(i) * model.A(:,:,i) * [rData(:,t-1); 1]; 
  end
  %Update position
...
%print('-dpng','graphs/demo_SEDS_augmSigma01.png');
pause;
close all;
