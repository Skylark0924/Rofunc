function demo_GMR_SEDS_discrete_augmSigma01
% Discrete autonomous dynamical system with augmented state-space encoding and  
% constrained optimization similar to the SEDS approach.
%
...
Data = [Data; ones(1,size(Data,2))]; %Augmented data
...
H = H ./ repmat(sum(H,2),1,model0.nbStates);    
...
rData0(:,1) = Data0; 
for t=2:nbData
	%rData0(:,t) = GMR(model0, rData0(:,t-1), [1:2], [3:4]); 
	rData0(:,t) = GMR(model, [rData0(:,t-1); 1], in, out); 
end
...
for t=2:nbData
  %Weight based on GMM
  for i=1:model.nbStates
    h(i,1) = model.Priors(i) * gaussPDF([rData(:,t-1); 1], model.Mu(in,i), model.Sigma(in,in,i));
  end
  h = h / sum(h);
  %Compute position through GMR
  for i=1:model.nbStates
    rData(:,t) = rData(:,t) + h(i) * model.A(:,:,i) * [rData(:,t-1); 1]; 
	end
end
...
for i=1:model0.nbStates
	plotGMM(model0.Mu(1:2,i), model0.Sigma(1:2,1:2,i), [.6 .6 .6], .8);
end
...
%print('-dpng','graphs/demo_SEDS_discrete_augmSigma01.png');
pause;
close all;
...
A = [A; zeros(1,size(A,2)-1), 1]; %Augmented state
%A = A(:,1:2); %Contractive system but moving equilibrium point
Atmp = (A+A')*.5;
